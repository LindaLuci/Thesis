...
\textit{Shared disk} ha due vantaggi: ogni processore ha la propria memoria, il bus di memoria non \`{e} un collo di bottiglia (contrariamente all'architettura "\textit{shared-everything}"). Inoltre il sistema offre un modo semplice per fornire un certo grado di tolleranza agli errori.\\
...
PostgreSQL \`{e} un potente sistema open source di database relazionale. \`{E} un sistema generale di gestione di database e di oggetti relazionali. Ha pi\`{u} di 15 anni di sviluppo attivo e un'architettura collaudata che ha guadagnato una notevole reputazione per l'affidabilit\`{a}, l'integrit\`{a} dei dati e la correttezza. \cite{etichetta12}

PostgreSQL è il primo sistema di gestione del database che implementa la funzionalit\`{a} di controllo della concorrenza multi-versione (\textit{Multi-Version Concurrency Control}, MVCC), anche prima di Oracle. La funzionalità MMVC è conosciuta come isolamento istantaneo in Oracle.

PostgreSQL è un sistema di gestione dei database relazionale object-relational. Permette di aggiungere funzioni personalizzate sviluppate utilizzando diversi linguaggi di programmazione quali C / C ++, Java, ecc.

PostgreSQL è progettato per essere estensibile. In PostgreSQL è possibile definire i propri tipi di dati, tipi di indici, lingue funzionali, ecc. Se non ti piace una parte del sistema, puoi sempre sviluppare un plugin personalizzato per ottimizzarlo per soddisfare le tue esigenze, ad esempio aggiungendo un nuovo ottimizzatore.

Se hai bisogno di un supporto, una comunità attiva è disponibile per aiutarti. È sempre possibile trovare le risposte della comunità di PostgreSQL per i problemi che si possono avere quando si lavora con PostgreSQL. Molte aziende offrono servizi di supporto commerciale in caso di necessità.
